...
namespace Yatt.Repo.Handlers
{
    public class TokenManager : ITokenManager
    {
        ...
        public async Task<string> GenerateToken(AuthDto user)
        {
            try
            {
        ...
        private async Task<List<Claim>> GetClaims(AuthDto user)
        {
            var claims = new List<Claim>
            {
                ...
                new Claim(ClaimConstant.PHONE_CONFIRMED, user.PhoneConfirmed.ToString()),
            };
            ...
            if (!string.IsNullOrEmpty(user.CompanyName))
                claims.Add(new Claim(ClaimConstant.COMPANY_NAME, user.CompanyName));
