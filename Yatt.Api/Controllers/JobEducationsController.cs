...
namespace Yatt.Api.Controllers
{
    ...
    [ApiController]
    public class JobEducationsController : ControllerBase
    {
        public IJobEducationRepository _repository { get; }

        public JobEducationsController(IJobEducationRepository repository)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] JobEducationDto model)
        {
            return Ok(await _repository.Create(model));
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Update(string id, [FromBody] JobEducationDto model)
        {
            if (id != model.Id)
